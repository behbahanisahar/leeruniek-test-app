  ...
  return (
    ...
    >
      {!Array.isArray(children) && (
        <div className={"card-body " + (fitBody ? "p-0" : "")}>{children}</div>
      )}
      {Array.isArray(children) && (
        <>
          {childrenType.header.status && (
            <>{children[childrenType.header.index]}</>
          )}
          {childrenType.content.status && (
            <div className={"card-body " + (fitBody ? "p-0 " : "")}>
              {children[childrenType.content.index]}
            </div>
          )}
